<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>租房管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图 -->
    <el-card>
      <el-row :gutter="20">
      	<!-- <el-checkbox-group v-model="queryInfo.querytype"> -->
      		<el-col :span="7">
      			<el-input placeholder="请输入内容" v-model="queryInfo.query.h_uid[0]" clearable @clear="getRentList">
      		...
      		<el-col :span="7">
      			...
      			<el-input placeholder="请输入内容" v-model="queryInfo.query.hid[0]" clearable @clear="getRentList">
      				<template slot="prepend">按房源编号查询</template>
      				<!-- <el-button slot="append" icon="el-icon-search" @click="getRentList"></el-button> -->
      			</el-input>
      		</el-col>
      		<el-col :span="2">
      			<!-- <el-link icon="el-icon-edit" @click="deleteMany">编辑</el-link> -->
      			<el-button type="primary" @click="getRentList" icon="el-icon-search" >查询</el-button>
      		</el-col>
      		<el-col :span="4">
      			<!-- <el-link icon="el-icon-edit" @click="deleteMany">编辑</el-link> -->
      ...
      <!-- 表格数据 -->
      <el-table :data="rentList" border stripe ref="multipleTable" @selection-change="handleSelectionChange" height="480">
        ...
        <el-table-column type="index" prop="index"></el-table-column>
		  <!-- 发布者信息 -->
		  <el-table-column sortable label="房源编号" prop="hid" width="100px"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getRentList () {
		 this.getAllhouseList();
      const { data: res } = await this.$axios.get('house/gethouselist', {
    ...
    async removeById (id) {
      ...
      if (confirmResult !== 'confirm') {
		...
		console.log('删除');
      const { data: res } = await this.$axios.get('house/delete',{params:{'hid':[id]}})
    ...
    handleSelectionChange(val) {
	 ...
	 topath: function(path,hid) {
	 	...
	 	const {data:res}= this.$axios.get('login/menu',
	 	 {
	 		 params:{'uid':this.queryInfo.uid}
	 	 }
	 	);
	 	const {data:response}= this.$axios.get('house/gethouse',
	 	 {
	 		 params:{'hid':hid}
	 	 }
	 	);
